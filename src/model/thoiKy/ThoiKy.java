...
//            String value = new String(arr, "UTF-8");
			...
			do {
				for (Element item : curr.select("div.items-leading > div:not(.advertise-box)")) {
					TrieuDai trieuDai = new TrieuDai(item.selectFirst(".page-header > h2 > a").text(),
							item.select("p:not(.readmore)").text());
				...
				if (result != null) {
					curr = Jsoup.connect("https://nguoikesu.com" + result.attr("href")).ignoreContentType(true)
							.timeout(0).get();
				}
			} while (result != null);
...
//            System.out.println(url);
	...
	public void setTrieuDai(ArrayList<TrieuDai> trieuDai) {
	...
	public void output() {
		...
		ArrayList<TrieuDai> jsonArray = (ArrayList<TrieuDai>) getTrieuDai();

		int n = jsonArray.size();
		for (int i = 0; i < n; i++) {
			TrieuDai TD = jsonArray.get(i);
			System.out.println("Trieu dai " + (i + 1) + ":");
			System.out.println("Ten: " + TD.getTen());
			System.out.println("Mo ta: " + TD.getMoTa());
			System.out.println("links: " + TD.getLinks());
		}
